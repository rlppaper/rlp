...
struct game_ui {
    ...
    int cur_point;
    long cur_x, cur_y; /* grid coordinates. */
    int cur_visible;
};
                            ...
                            int x, int y, int button)
{
    ...
    if (IS_CURSOR_MOVE(button) && ui->dragpoint < 0)
    {
        x = ui->cur_x;
        y = ui->cur_y;
        ui->cur_visible = 1;

        int dirx = (button == CURSOR_RIGHT) ? +1 : (button == CURSOR_LEFT) ? -1 : 0;
        int diry = (button == CURSOR_DOWN) ? +1 : (button == CURSOR_UP) ? -1 : 0;
        int dorthx = 1 - abs(dirx), dorthy = 1 - abs(diry);
        int i, best;
        long bestd, px, py;
        ...
        for (i = 0; i < n; i++)
        {
            ...
            /* only consider points that lie in the
             * 90° cone centered on the cursor move direction
             *                CURSOR_UP
             *                 \   /
             *                  \ /
             *  CURSOR_LEFT      o     CURSOR_RIGHT
             *                  / \
             *                 /   \
             *              CURSOR_DOWN
             */
            if ((dorthx && (diry * (ui->cur_y - py) >= 0 || absdy < absdx)) ||
                (dorthy && (dirx * (ui->cur_x - px) >= 0 || absdx < absdy)))
            {
                continue;
            }
        ...
        if (best >= 0)
        {
            px = state->pts[best].x * ds->tilesize / state->pts[best].d; ui->cur_x = state->pts[best].x;
            py = state->pts[best].y * ds->tilesize / state->pts[best].d; ui->cur_x = state->pts[best].x;
            ui->cur_x = px;
            ui->cur_y = py;
            ui->cur_point = best;
        }
    ...
    else if (IS_CURSOR_MOVE(button) && ui->dragpoint >= 0)
    {
        if (!ui->cur_visible)
        {
            ui->cur_visible = 1;
        ...
        int dirx = (button == CURSOR_RIGHT) ? +1 : (button == CURSOR_LEFT) ? -1 : 0;
        int diry = (button == CURSOR_DOWN) ? +1 : (button == CURSOR_UP) ? -1 : 0;
        int halftilesize = ds->tilesize / 2;
        ui->newpoint.x += dirx * halftilesize;
        ui->newpoint.y += diry * halftilesize;
        ui->newpoint.d = ds->tilesize;

        if (ui->dragpoint == ui->cur_point)
        {
            ui->cur_x = ui->newpoint.x;
            ui->cur_y = ui->newpoint.y;
        }
        return UI_UPDATE;
    }
    else if (IS_CURSOR_SELECT(button) && ui->dragpoint < 0)
    {
        if (!ui->cur_visible)
        {
            ui->cur_visible = 1;
        ...
        ui->dragpoint = ui->cur_point;
        ui->newpoint.x = ui->cur_x;
        ui->newpoint.y = ui->cur_y;
        ui->newpoint.d = ds->tilesize;
        return UI_UPDATE;
    }
    else if (IS_CURSOR_SELECT(button) && ui->dragpoint >= 0)
	...
	best = -1;
	bestd = 0;
    ui->dragpoint = -1; /* reset in case the cursor selected a point */
    ui->cur_visible = 0;
...
finalizemove:;
    ...
    {
        ui->cur_x = state->pts[p].x * ds->tilesize / state->pts[p].d;
        ui->cur_y = state->pts[p].y * ds->tilesize / state->pts[p].d;
        return UI_UPDATE;
    }
                        ...
                        float animtime, float flashtime)
{
    ...
    if (ds->bg == bg && ds->dragpoint == ui->dragpoint && ds->cur_point == ui->cur_point && !points_moved)
        return;                        /* nothing to do */

    ds->dragpoint = ui->dragpoint;
    ds->bg = bg;
    ds->cur_point = ui->cur_point;
	...
	for (i = 0; i < state->params.n; i++) {
		...
		c = COL_NEIGHBOUR;
	    } else if (ui->cur_point == i && ui->cur_visible == 1) {
        c = COL_CURSOR;
        } else {
		c = COL_POINT;
	    }
